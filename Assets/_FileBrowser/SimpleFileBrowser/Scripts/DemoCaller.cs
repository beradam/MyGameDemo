...
namespace GracesGames.SimpleFileBrowser.Scripts {
	...
	public class DemoCaller : MonoBehaviour {
		...
		public string[] FileExtensions = { "txt" };

        // MyCode
        private string[] fileData;

        public bool mehetunketovabb = false;
		...
		private void OpenFileBrowser(FileBrowserMode fileBrowserMode) {
			...
			GameObject fileBrowserObject = Instantiate(FileBrowserPrefab);
		...
		private void LoadFileUsingPath(string path) {
			if (path.Length != 0) {
                ...
                //fileData = bFormatter.Deserialize(file) as string;
                // We're done working with the file so we can close it
                //file.Close();
                // MyCode
                fileData = File.ReadAllLines(path);
                mehetunketovabb = true;
